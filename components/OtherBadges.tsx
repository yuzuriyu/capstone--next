...
interface OtherBadgesProps {
  userData: User;
}

const OtherBadges: React.FC<OtherBadgesProps> = ({ userData }) => {
  const { totalAccumulatedVoltage, totalSteps } =
    useContext(OtherVoltageContext);
  const [isAchievementsOpen, setIsAchievementsOpen] = useState(false);
  const [badges, setBadges] = useState<BadgeType[]>(userData?.badges || []);
