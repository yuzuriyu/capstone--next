...
const OtherProfile = () => {
  ...
  useEffect(() => {
    if (!selectedUser && (!badgeContext || !badgeContext.badges)) {
      // Fetch badges or handle initialization here
    }
  }, [selectedUser, badgeContext]);
      ...
      <div className="bg-bggray">
        ...
        <div className="w-11/12 m-auto py-8 flex flex-col lg:flex-row gap-4">
          <div className="md:w-1/4 w-full mt-16">
            ...
            <p className="text-sm mb-4 text-gray-500">{selectedUser?.title}</p>
          ...
          <div className="rounded-lg bg-white py-8 px-8 flex-1">
            ...
            {!isEditBioOpen && (
              <p className="text-gray-500 py-4">{selectedUser?.bio}</p>
            )}
            {isEditBioOpen && (
              <div className="my-4">
                <input
